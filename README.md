...
현재 구현된 단위는 backend retrieval core prototype, Phase 1A frontend shell, Phase 1B live Search Console, Phase 1C Knowledge Library API/UI, Phase 2 fake answer layer, Phase 3 fake eval runner, query log 기반 Operations API, top evidence persistence, eval persistence, Operations query trend/detail, opt-in OpenAI Responses API transport, 한국어 데모 UI와 최신 screenshot assets입니다.

- FastAPI application skeleton: `GET /health`, `POST /documents`, `GET /documents`, `GET /documents/{document_id}`, `POST /retrieve`, `POST /answer`, `POST /eval-runs`, `GET /eval-runs`, `GET /metrics/summary`, `GET /metrics/trend`, `GET /queries/recent`, `GET /queries/{query_id}`, `GET /evidence/top`
...
다음 구현은 Vercel GitHub App repo access 승인 후 자동 배포 연결, production auth/SSO, 또는 더 깊은 admin workflow 중 하나로 나눌 수 있습니다. 기본 로컬/CI 경로는 계속 API key 없이 fake provider로 동작합니다.
