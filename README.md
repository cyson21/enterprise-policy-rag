...
- Auth UI status: top bar shows the current auth session mode while keeping persona selection for demo scenarios. Default `AUTH_CONTEXT_PROVIDER=demo` does not enforce auth and logs a warning; production backends must use `AUTH_CONTEXT_PROVIDER=trusted_headers|oidc_jwt` or `RAG_REQUIRE_AUTH=1`.
