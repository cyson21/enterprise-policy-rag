...
1차 범위에서는 온프레미스 설치형 배포를 제외하고, 로컬 Docker Compose로 재현 가능한 백엔드와 포트폴리오 시연용 데모 화면을 함께 만듭니다.

## Scope Boundary (재개 포인트)

- 현재 문서의 기준은 **포트폴리오 시연·검증 범위**이며, 운영 전환 완료를 뜻하지 않습니다.
- Public 데모는 static read-only 경로( `VITE_DEMO_MODE=static` )를 기준으로, fixture 기반 화면 렌더링으로 확인합니다.
- `AUTH_CONTEXT_PROVIDER=demo`는 기본 데모 auth입니다. 운영형 auth는 `AUTH_CONTEXT_PROVIDER=trusted_headers|oidc_jwt`와 `RAG_REQUIRE_AUTH=1` 조합을 별도 경로로 유지합니다.
- OpenAI 연동은 `EMBEDDING_PROVIDER=openai`/`LLM_PROVIDER=openai` 혹은 `RUN_OPENAI_LIVE_SMOKE=1` 설정일 때만 opt-in됩니다.
- 운영 전환 보증, SLA/SLO, 스케일링 실측 같은 생산 운영 주장 없이 하드닝 항목은 별도 트랙으로 분리해 추적합니다.
...
| Cache/Queue | Redis |
| LLM/Embedding | fake provider first, env-gated OpenAI adapters |
...
- Auth/SSO boundary: default demo auth context (`AUTH_CONTEXT_PROVIDER=demo`), opt-in trusted-header context, opt-in OIDC JWT context, and session-bound retrieval/answer endpoints
...
- Provider boundary: `EmbeddingProvider`, `LLMProvider`, deterministic `FakeEmbeddingProvider`, deterministic `FakeLLMProvider`, env-gated `OpenAIEmbeddingProvider`, env-gated `OpenAILLMProvider`
- Embedding provider selection: default `EMBEDDING_PROVIDER=fake`; `EMBEDDING_PROVIDER=openai` requires `OPENAI_API_KEY`, uses `OPENAI_EMBEDDING_MODEL` default `text-embedding-3-small`, and pins `dimensions=64` for the `vector(64)` schema
...
- OpenAI timeout: `OPENAI_TIMEOUT_SECONDS` applies to OpenAI Responses and Embeddings calls; default is 20 seconds
...
- Local retrieval path: in-memory repository + fake embeddings; PostgreSQL retrieval uses pgvector candidate top-k pushdown when `DATABASE_URL` is set
...
- Auth UI status: top bar shows the current auth session mode while keeping persona selection for demo scenarios. Default `AUTH_CONTEXT_PROVIDER=demo` does not enforce auth and logs a warning; production backends can switch to `AUTH_CONTEXT_PROVIDER=trusted_headers|oidc_jwt` with `RAG_REQUIRE_AUTH=1`.
...
OpenAI retrieval/answer 경로는 명시적으로 provider를 선택할 때만 활성화됩니다.
...
| [Production Hardening Checklist](docs/runbooks/production-hardening-checklist.md) | 운영 전환 전 필수 보강 항목 |
