...
현재 구현된 단위는 backend retrieval core prototype, Phase 1A frontend shell, Phase 1B live Search Console, Phase 1C Knowledge Library API/UI, Phase 2 fake answer layer, Phase 3 fake eval runner, query log 기반 Operations API, top evidence persistence, eval persistence, Operations query trend/detail, opt-in OpenAI Responses API transport, production auth/SSO boundary, 한국어 데모 UI와 최신 screenshot assets입니다.

- FastAPI application skeleton: `GET /health`, `POST /documents`, `GET /documents`, `GET /documents/{document_id}`, `POST /retrieve`, `POST /answer`, `GET /auth/session`, `POST /auth/retrieve`, `POST /auth/answer`, `POST /eval-runs`, `GET /eval-runs`, `GET /metrics/summary`, `GET /metrics/trend`, `GET /queries/recent`, `GET /queries/{query_id}`, `GET /evidence/top`
...
- Schema: workspace, document, document chunk, query log, retrieval result, answer, citation, eval run, eval case result, pgvector embedding column
...
다음 구현은 admin workflow, real IdP/OIDC adapter, 또는 controlled live OpenAI smoke 중 하나로 나눌 수 있습니다. 기본 로컬/CI 경로는 계속 API key 없이 fake provider로 동작합니다.
...
| 문서 | 내용 |
|---|---|
| [Project Tracking](docs/project-tracking.md) | 현재 상태, 결정, phase별 산출물 |
...
| [Product Architecture](docs/internal/design/2026-05-20-enterprise-policy-rag-product-architecture.md) | 전체 아키텍처, 화면 구성, phase plan |
| [Master Implementation Plan](docs/internal/plans/2026-05-20-enterprise-policy-rag-master-plan.md) | 전체 phase, 화면, API, 데이터, 검증 상세 계획 |
| [Architecture-First UI Plan](docs/internal/plans/2026-05-20-architecture-first-ui-implementation-plan.md) | 화면 포함 구현 순서와 완료 기준 |
| [Foundation Plan](docs/internal/plans/2026-05-20-project-02-foundation-plan.md) | 초기 계획과 범위 |
...
| [Portfolio Interview Guide](docs/portfolio-interview-guide.md) | 시연 순서, 아키텍처 설명, 트레이드오프 |
| [Architecture SVG](docs/assets/architecture.svg) | 아키텍처 다이어그램 |
| [Next Agent Bootstrap](docs/next-agent-bootstrap.md) | 다음 에이전트 시작 지침 |

## Working Rule

이 레포에서 실제 구현을 시작하는 에이전트는 먼저 `docs/next-agent-bootstrap.md`, `docs/project-tracking.md`, `TODO.md`를 읽고 현재 범위를 확인합니다.
